...
**Fire:** 2026-08-01 ~18:30 local (EEST)  
**Law:** commit every fix · workshop only after smoke · mat_queue=1 · cube_is_source_of_truth · no dual-truth pose · primary-hand SoT

## Sources
- Comments: **1256** (+3 vs last fire) — **Kriminull** forced walk / no sprint (new); author Cube prophecy note; Батон praise
- Bugs ctp=24: still **349** — #346 fly · #347 black HMD · #348 flicker · #349 HUD ghost
- Questions: 104 — no new last-page signal this fire (help thread first page only)
...
|--------|--------|---------|
| **Forced walk / cannot sprint** | **Kriminull** (~2h) | **Fixed this fire** — speed SoT + auto-sprint + clear stuck +walk + Knuckles/Cosmos stick-click → sprint |
...
## Actions this fire
- Re-fetched comments (1256) + bugs last page (349) + GH issues + QUESTIONS pin.
- **Code fix:** locomotion sprint SoT (`sh_locomotion.lua`), `vrmod_autosprint` convars, Knuckles/Cosmos bindings (no force-position walk on stick), Controls catalog rows.
- Commit + **push origin/main**.
- **No gmpublish** until smoke of sprint + pending menu/HUD stack.
...
## Next loop
Only new actionable Lua after smoke; ship bar remains smoke of fixed stack + this sprint fix.
