...
-- Start range: 12 (gVRMod) was too tight for many stock VMs; 20 was solid; 28 is forgiving
local GRIP_DISTANCE = 28
-- Two-hand aim blend (gVRMod uses 0.45; mild enough to keep RH primary)
local GUIDE_BLEND = 0.35
...
local function UiBlockingGrip()
	-- Only block stock foregrip while actively resizing a panel, or free-grabbing
	-- a laser-focused menu. Stuck menuGrabActive without focus must not kill FG.
	if g_VR.menuResizeActive then return true end
	if g_VR.menuGrabActive and g_VR.menuFocus then return true end
	return false
end
...
local function TryStartGrip()
	...
	if UiBlockingGrip() then return false end
...
--- cl_input calls this before prop pickup / menu grab.
...
-- Backup input path (cl_input is primary). Do not double-process if already handled.
hook.Add("VRMod_Input", "vrmod_foregrip", function(action, pressed)
	if not g_VR or not g_VR.active then return end
	if action ~= "boolean_left_pickup" then return end
	if UiBlockingGrip() then
		if not pressed and state.gripping then ClearGrip() end
		return
	end
	-- If cl_input already called TryForegripGrab this frame, still safe (idempotent)
