...
vrmod.SettingsCatalog = {
	...
	},
	{
		...
		rows = {
			...
			{ kind = "header", label = "Stereo / head tilt" },
			...
			{ kind = "help", label = "Both eyes share one HMD orientation (roll). Mode 1 = headset eye poses. Mode 2 = translate along head Right() only. Prefer 0 or 1; leave crop on 0 if the world stretches when you tilt." },
			{ kind = "slider", label = "Synthetic IPD scale", cvar = "vrmod_eyescale", min = 0.0, max = 1.0, decimals = 2 },
			{ kind = "help", label = "Only when eyes are synthetic (mode 2 or auto fallback): 1.0 = full IPD, 0.5 = half. Wrong depth/stretch on tilt → try mode 1 + eyescale 1.0." },
