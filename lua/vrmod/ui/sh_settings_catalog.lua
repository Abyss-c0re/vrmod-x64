	...
	},
	{
		...
		rows = {
			...
			{ kind = "combo", label = "Stereo eye placement", cvar = "vrmod_stereo_eye_mode", choices = {
				{ text = "0 — auto (XR then synthetic)", value = 0 },
				{ text = "1 — OpenXR eye poses", value = 1 },
				{ text = "2 — synthetic head-Right IPD", value = 2 },
			}},
			{ kind = "help", label = "Head-tilt dial. Mode 2 = pure IPD along head right (best roll test). Shared HMD orientation always." },
			...
			{ kind = "help", label = "Leave on 0. Mode 2 can cross stereo (do not leave on)." },
			...
			{ kind = "help", label = "Only if stereo is inverted. Default off. Content halves only — not a warp fix." },
			...
			{ kind = "slider", label = "Synthetic IPD scale", cvar = "vrmod_eyescale", min = 0.0, max = 1.0, decimals = 2 },
			{ kind = "help", label = "When eyes are synthetic: 1.0 = full IPD, 0.5 = half. Try 1.0 if roll looks flat/wrong depth." },
			{ kind = "slider", label = "Scale factor", cvar = "vrmod_scalefactor", min = 0.05, max = 4.0, decimals = 2 },
			{ kind = "help", label = "Submit UV border crop. Nudge ±0.05 if edges warp without changing eyes." },
