...
if CLIENT then
	...
	local function EnableClimbing()
		...
		local targetPoses = {
			{
				controllerPoseName = "pose_lefthand",
				setHandPoseFunc = vrmod.SetLeftHandPose,
				time = 0
			},
			{
				controllerPoseName = "pose_righthand",
				setHandPoseFunc = vrmod.SetRightHandPose,
				time = 0
			}
		}
			...
			if action == "boolean_left_pickup" or action == "boolean_right_pickup" then
				...
				elseif refHand then
					...
					if targetPoses[3 - hand].hold then
						...
						for k = 1, #dismounts do
							local v = dismounts[k]
							local dot = g_VR.tracking.hmd.ang:Forward():Dot(((v + Vector(0, 0, 18)) - g_VR.tracking.hmd.pos):GetNormalized())
							if math.abs(v.z - g_VR.origin.z) < 60 and dot > dismountDot and util.TraceLine({
								start = g_VR.tracking.hmd.pos,
								endpos = v,
								filter = LocalPlayer()
							}).Fraction > 0.99 then
								dismountPos = v
	...
	hook.Add("VRMod_Start", "climbing", function() if convars.vrmod_climbing:GetBool() then EnableClimbing() end end)
	hook.Add("VRMod_Exit", "climbing", function() if convars.vrmod_climbing:GetBool() then DisableClimbing() end end)
elseif SERVER then
		...
		if value ~= "vrmod" and ent:GetClass() == "func_useableladder" and (input == "Enable" or input == "Disable") then
			for k, v in pairs(vrmod_ladders[game.GetMap()]) do
				if (ent:LocalToWorld(ent:GetKeyValues().point1) - v.pos):Length2DSqr() < 1024 then
					...
					if ent.vrmod_shouldenable and climbingEnabled then return true end
				end
			end
		end
	end)
	...
	net.Receive("vrmod_enableclimbing", function(len, ply)
		...
		if climbingEnabled then
			...
			for k, v in pairs(ents.FindByClass("func_useableladder")) do
				if v.vrmod_shouldenable then v:Fire("Enable", "vrmod") end
			end
		end
	end)
